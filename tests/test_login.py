...
def test_login_success(driver, creds):
    ...
    assert page.is_on_fan_hudl(), "Expected to be on fan.hudl.com after valid login."

# logout 
def test_logout_after_login(driver, creds):
    page = HudlLoginPage(driver)
    page.login(creds["email"], creds["password"])
    assert page.is_on_fan_hudl(), "Precondition failed: not on fan.hudl.com."
    page.logout()
...
def test_login_wrong_password_shows_error(driver, creds):
    ...
    assert not page.is_on_fan_hudl(), "Should not be on fan.hudl.com with wrong password."
...
def test_login_invalid_email_format(driver):
    ...
    assert page.is_on_login_screen(), "Invalid email should keep you on login screen with email field visible."
...
def test_login_empty_email_field(driver):
    ...
    assert page.is_on_login_screen(), "Empty email should keep you on login screen with email field visible."
...
def test_login_empty_password(driver, creds):
    ...
    page.enter_email(creds["email"])
    page.click(page.CONTINUE_BUTTON)
    assert not page.is_on_fan_hudl(), "Empty password must not log you in to fan.hudl.com."
